...
```
或是從原圖片之四個角落點及中心點crop,在翻轉可以產生10張擴增數據,如/caffe/python/caffe/io.py 中之oversample

**- Random Scale : **隨機尺度變換
**- Random Rotation :** 隨機小尺度旋轉
**- Noise : **增加高斯噪音或模糊處理
**- Label Shuffle : **對於各類別數據量不均等情況，可參考海康威視ILSVRC2016，藉由選取最大數據量類別總量當基準，隨機擴增其他類別總量，使其產生類別數據量均等之訓練列表
**- Random Blocking :** 可製造隨機比例水平或是垂直遮擋圖像，造成物體不完整來訓練增加魯棒性

## 圖像預處理：
**- 隨機遮擋補齊：**在人臉檢測或是物件檢測中，當出現圖像邊緣檢測框時，為維持同等訓練集圖片比例可增加無信息margin像素，所使用之補齊margin像素並非0或255，而是使用通過零中心所使用之mean＋std，抑或是製造隨機遮檔訓練集數據時可用同方法來當遮蔽物

## 多模型融合
**- 多模型加權平均：**可使用resnet152，densenet201,resnext101等模型分開訓練，在預測時加權平均其所有模型結果，此權重維持為1即可，並非所有模型權重皆相同，可由驗證集來測試求取最佳比例

## 改彩色為單調
**- 改彩色為單調：**若識別任務對色彩影響不重要，如人臉某些屬性或是汽車車型等屬性識別，則可改rgb三通到為灰階單通道，可避免過擬合及增加訓練速度

## 卷積權重固定
**- 預訓練模型卷積權重固定：**使用imagenet預訓練模型時，可固定卷積權重使其維持特徵擷取能力，在進行微調。尤其當數據集不夠時此作法比從頭訓練或是不固定權重微調結果更好

## 訓練超參數
**- weight decay選擇：**對於不同複雜模型而言，皆有著最佳之weight decay參數，一般為5x10^-4，可往1x10^-4測試調整
